...
defaultStation = { tiles = tileRect (-3, -3) (3, 3) }

tileRect : (Int, Int) -> (Int, Int) -> D.Dict (Float, Float) Tile
tileRect (x1, y1) (x2, y2) =
    let (lx, ux) = if x1 > x2 then (x2, x1) else (x1, x2)
        (ly, uy) = if y1 > y2 then (y2, y1) else (y1, y2)
    in   D.fromList <| foldl (\xp tiles -> concat 
                       [ map (\yp -> ((toFloat xp, toFloat yp), {tileType = 1})) [ly..uy]
                       , tiles ])
                     [] [lx..ux]
...
addToStation station cursor =
    case cursor of
      Position _      -> station
      Selection p1 p2 -> { station | tiles <- D.union station.tiles <| tileRect p1 p2 }
